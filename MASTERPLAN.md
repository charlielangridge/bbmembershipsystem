# BB Membership System Modernisation Masterplan

Status: Phase 1 repository foundation complete; Cloud staging proof deferred until an environment is provisioned; parity implementation in progress
...
- The rebuild remains isolated from legacy production; see [ADR 0004](docs/adr/0004-isolate-the-rebuild-from-legacy-production.md).
- The replacement targets MySQL 8.4 and receives legacy data through an explicit converter; see [ADR 0005](docs/adr/0005-target-mysql-84-with-an-explicit-legacy-converter.md).
- Inertia Vue owns public/member journeys; see [ADR 0006](docs/adr/0006-use-inertia-vue-for-member-journeys.md).
- Production targets Laravel Cloud with Valkey-backed workers; see [ADR 0007](docs/adr/0007-run-on-laravel-cloud-with-valkey-backed-workers.md).
- Flare reports errors without member identity; see [ADR 0008](docs/adr/0008-report-errors-to-flare-without-member-identity.md).
...
### Phase 1 foundation checkpoint — 1 September 2026
...
- verified Pest, Larastan, Pint, Vue type checking, ESLint, Prettier, the production Vite build, and MySQL 8.4 schema/tests in [GitHub Actions run 33486265465](https://github.com/charlielangridge/bbmembershipsystem/actions/runs/33486265465) on commit `4789aaa`;
- added a production-like Sail topology using PHP 8.4, MySQL 8.4, Valkey, Mailpit, and an independent queue worker while retaining fast SQLite tests;
- made CI migrate the complete schema on MySQL 8.4 before running Pest and the existing dependency, formatting, static-analysis, frontend, and build gates;
- retained Laravel's `/up` liveness endpoint, added UUID request correlation across HTTP responses/log context/queued jobs, and verified that health responses do not expose configuration;
- installed Flare with reporting disabled by default and privacy controls that exclude client IPs, cookies, sessions, credentials, and member identity;
- selected Laravel Cloud for eventual production and documented staging setup, deployment, verification, and rollback without provisioning chargeable resources.

Phase 1's repository work is complete. Its live staging deployment/rollback evidence remains deliberately deferred because the client confirmed that the application is not currently deployed to Laravel Cloud; this evidence is a production-readiness gate, not a fabricated completion claim.
...
| Inertia Vue frontend foundation | Complete | Inertia 3, Vue 3, TypeScript, Tailwind 4, Vite, and Wayfinder are installed and build successfully; protected account and member-directory pages are the first ported slices. |
| Authentication foundation | Browser and legacy-identity slice complete; API work pending | Fortify owns all human flows through the `web` guard. The legacy identity preflight/dry-run/import preserves compatible IDs, hashes, timestamps, and explicit verification cutover state; imported bcrypt passwords authenticate through Fortify and upgrade. Sanctum remains deferred until the first approved API/mobile slice. |
| Permissions and administration | Initial slice complete | Spatie Laravel Permission is authoritative, legacy roles/assignments have a tested import path, policies enforce member/account reads, and Filament has an explicit privileged access boundary. Additional domain permissions/resources arrive with their vertical slices. |
...
| Continuous integration | Complete | A clean GitHub runner passed MySQL 8.4 migration plus the complete quality job in run 33486265465 on commit `4789aaa`. Branch-protection policy remains an administrative repository task. |
| Local production-like services | Complete for Phase 1 | Sail pins PHP/MySQL 8.4 with Valkey, Mailpit, and a separate Redis queue worker. Storage tests use Laravel fakes; Cloud object storage and its S3 adapter are deferred until the first upload deployment slice. |
...
| Staging, observability, and deployment | Repository-ready; live proof deferred | Laravel Cloud is selected; Flare, protected logs, correlation IDs, and the Cloud release/rollback runbook are implemented. No Cloud environment exists yet, so provisioning, secrets, a staging release, and rollback evidence remain future production-readiness gates. |
| Business logic and external integrations | In progress | Account self/admin reads, member directory/privacy, private photos, legacy identities, roles, and the Filament boundary are implemented. Membership writes, finance, access control, equipment, providers, schedules, Fathom, and Reverb remain. |
...
3. Add the official Laravel 13 Inertia Vue starter-kit skeleton without legacy application code.
...
Do not combine foundation work with copied legacy models/controllers. The commits after `d686bf6` and before the first business-logic slice are the completed clean-foundation audit reference.
...
| Database | MySQL 8.4 | New schema targets MySQL 8.4; an explicit converter bridges the still-unknown legacy production engine/schema |
...
| Queues | Redis driver on Laravel Valkey with dedicated Cloud worker compute | Dispatch after commit; all queued handlers idempotent; managed queues require a later explicit approval |
| Storage | Private local disk/fakes now; Laravel Cloud Object Storage before upload deployment | Install the S3 adapter and prove authorised private access before member uploads run on Cloud |
...
Status on 1 September 2026: repository implementation complete. Live Laravel Cloud staging deployment and rollback proof are deferred until a staging environment is intentionally provisioned; see [the Cloud release runbook](docs/runbooks/laravel-cloud-release.md).
...
- Demonstrate staging deployment of the skeleton and rollback to its prior release when Cloud staging is provisioned.
...
- clean Laravel 13/PHP 8.4 checkout and documented native/Sail setup;
- MySQL 8.4 schema migration in CI plus Pest, Pint, Larastan, Composer/npm audits, Vue type checking, ESLint, Prettier, and Vite build;
- safe `.env.example`, `/up`, request correlation, protected logging, and privacy-minimised Flare adapter;
- ADRs 0004–0008 and a no-secrets Laravel Cloud deploy/rollback runbook;
- no Cloud resources or production identifiers created because deployment is explicitly eventual, not current.
...
| Replace implementation | Preserve the outcome without preserving obsolete internals | Old Stripe Checkout, unsupported PayPal IPN SDK, legacy Pusher/GIF/Swagger/log-viewer packages, Universal Analytics/Google+ implementation |
...
| NEW-004 | ~~Generate clean Laravel 13 root skeleton~~ | S | Complete: conventional root skeleton runs on PHP 8.4 |
| NEW-005 | ~~Add reproducible local environment~~ | M | Complete: native SQLite and production-like Sail paths are documented |
| NEW-006 | ~~Add CI quality pipeline~~ | M | Complete: MySQL migration, tests, Pint, Larastan, audits, frontend checks, and Vite are gated |
...
## 17. Decisions and production facts still to close
...
4. ~~Existing database compatibility versus a planned transformed schema.~~ Resolved 1 September 2026: target a new MySQL 8.4 schema and build an explicit, reconciled converter after the legacy production facts are obtained.
...
6. ~~CI, staging, production hosting, queue, cache/session, monitoring, and secret-management platforms.~~ Resolved 1 September 2026: GitHub Actions; eventual Laravel Cloud; Valkey-backed Redis queues/cache/encrypted sessions; dedicated workers; Flare; Laravel Cloud secrets. Region, sizing, budget, mail, storage, and environment identifiers remain provisioning facts.
...
## 18. Current handoff and next execution order

The branch is `modernisation/laravel-13` in `charlielangridge/bbmembershipsystem`. The candidate legacy source revision is `d686bf6`, but the deployed production revision, database, infrastructure, providers, and device clients remain unconfirmed. Do not access or change the fragile hosted application without an explicit production-information handoff.

Completed vertical slices after the skeleton:

1. legacy database connectivity preflight and identity dry-run/import;
2. Fortify authentication of imported bcrypt identities with hash upgrade;
3. Spatie role/permission import and policy boundary;
4. protected account read;
5. Filament administration access boundary;
6. privacy-aware member directory and authorised private photo delivery;
7. Phase 1 local/CI/observability/Cloud-ready foundation.

Next execution order:

1. Obtain the deployed legacy commit, structure-only schema, sanitised snapshot, engine/version, data volumes, current provider/device configuration, and named business owners.
2. Complete Milestone 2 by comparing that evidence with the candidate-source ledgers and building the repeatable source-to-MySQL converter plus reconciliation reports.
3. Finish Milestone 3 identity/member read parity against the production-shaped snapshot, including unresolved role collisions and primary-key sequence reconciliation.
4. Implement Milestone 4 membership lifecycle/write journeys as Pest-tested vertical slices in Inertia Vue and Filament.
5. Provision Cloud staging only when region, budget, sizing, mail, object storage, Flare project, and release ownership are approved; then execute and record the release/rollback runbook.

## 19. Historical audit summary

The legacy audit found a Laravel 5.1-era application with obsolete dependencies, committed/generated assets, mixed responsibilities in controllers/models, thin automated coverage, custom authentication/integration code, and high operational fragility. Its broad upgrade-in-place phases were superseded by the client's clean-skeleton decision because transplanting characterised behaviour into a supported Laravel application is safer than repeatedly modifying the hosted system.

The full original audit and the 27 August checkpoint remain under `docs/archive/` as historical evidence. They are not active plans; this file is the only document to update for programme status, remaining work, or handoff.
